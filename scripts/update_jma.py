...
import json, re
JST=timezone(timedelta(hours=9)); UA={'User-Agent':'JapanDisasterDashboard/6.0'}
...
def sev_name(name):
 if '特別警報' in name or '大津波警報' in name or '氾濫発生' in name or '緊急安全確保' in name:return 4
 if '危険警報' in name or '津波警報' in name or '氾濫危険' in name or '避難指示' in name:return 3
 if '土砂災害警戒情報' in name or '氾濫警戒' in name:return 2
 if '警報' in name and '注意報' not in name:return 3
 if '注意報' in name or '氾濫注意' in name or '高齢者等避難' in name:return 1
 return 0
def label(lv):return '緊急' if lv>=4 else '警報' if lv>=3 else '警戒' if lv>=2 else '注意' if lv>=1 else '平常'
def active_kinds(area):
 out=[]
 for w in area.get('warnings',[]):
  if str(w.get('status','')) in ('解除','発表警報・注意報はなし','解除済み'):continue
  n=w.get('name') or w.get('kind') or ''
  if n and ('警報' in n or '注意報' in n):out.append(n)
 return list(dict.fromkeys(out))
def parse_warning(pref,code):
 ...
 items=sum((x['items'] for x in municipal.values()),[]);items=list(dict.fromkeys(items));lv=max([x['level'] for x in municipal.values()] or [0])
...
def feed_entries(url):
 ...
 return out
def apply(prefs,pnames,lv,kind,detail):
 for p in pnames:
  if p not in prefs:continue
  prefs[p]['hazards'][kind]={'level':lv,'detail':detail}
  if lv>prefs[p]['level']:prefs[p]['level']=lv;prefs[p]['dominant']=kind;prefs[p]['detail']=detail
def parse_xml_hazards(prefs):
 # Recent JMA XML feeds: earthquake/tsunami from eqvol; landslide/flood from extra.
 for feed,limit in ((FEEDS['eqvol'],100),(FEEDS['extra'],180)):
  ...
  for ent in entries:
   ...
   if '津波' in title:
    ...
    apply(prefs,pnames,lv,'津波',detail)
   elif '土砂災害警戒情報' in title:
    # XML contains warning/解除 municipalities; only count bulletins that contain active warning wording.
    if '解除' in txt and not ('警戒対象地域' in txt or '警戒' in txt.replace('解除','')):continue
    apply(prefs,pnames,2,'土砂','土砂災害警戒情報')
   elif '洪水' in title or '氾濫' in title:
    hits=[x for x in ['氾濫発生情報','氾濫危険情報','氾濫警戒情報','氾濫注意情報'] if x in txt]
    if hits:apply(prefs,pnames,max(sev_name(x) for x in hits),'河川',hits[0])
   elif '震度' in title:
    vals=[]
    for x in root.iter():
...
def main():
 ...
 Path('data').mkdir(exist_ok=True);Path('data/jma.json').write_text(json.dumps({'updated':now,'prefectures':prefs,'regions':regions,'sources':['JMA warning JSON','JMA disaster XML'],'note':'Prefecture tab color is maximum severity among weather, earthquake, tsunami, landslide and river. Evacuation is shown as unconnected until a reliable machine-readable feed is configured.'},ensure_ascii=False,indent=2),encoding='utf-8')
