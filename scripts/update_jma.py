...
import json
JST=timezone(timedelta(hours=9)); UA={'User-Agent':'JapanDisasterDashboard/8.0'}
...
def sev_name(n):
 # 2026 terminology: level5 landslide special warning, level4 landslide danger warning, level3 landslide warning, level2 landslide advisory.
 if '土砂災害特別警報' in n or '特別警報' in n or '大津波警報' in n or '氾濫発生' in n:return 4
 if '土砂災害危険警報' in n or '津波警報' in n or '氾濫危険' in n:return 3
 if '土砂災害警報' in n:return 2
 if '土砂災害警戒情報' in n:return 3  # legacy name corresponds to current level4 landslide danger warning
 if '氾濫警戒' in n:return 2
 if '警報' in n and '注意報' not in n:return 3
 if '土砂災害注意報' in n or '注意報' in n or '氾濫注意' in n:return 1
 return 0
...
def parse_warning(code):
 ...
 for a in areas:
  ...
  if c:municipal[c]={'name':a.get('name',''),'items':kinds,'level':max([sev_name(x) for x in kinds] or [0])}
...
def active_landslide_areas(root):
 # Extract municipality names from active Warning/Item blocks; avoid treating an entire prefecture as active because its name appears in the bulletin header.
 names=[]
 ...
 for feed,limit in ((FEEDS['eqvol'],100),(FEEDS['extra'],180)):
  ...
  for ent in entries:
   ...
   if '津波' in title:
    ...
    lv=sev_name(detail)
    apply(prefs,pnames,lv,'土砂',detail,ent.get('updated'))
    if '福島県' in pnames:
     fukushima_landslide += active_landslide_areas(root)
...
def main():
 ...
 # Promote affected Fukushima municipalities with an active landslide bulletin, then aggregate municipality -> 7 regions.
 for code,d in fmun.items():
  if d.get('name') in fukushima_landslide:
   lv=3;d['level']=max(d.get('level',0),lv);d['items']=list(dict.fromkeys(d.get('items',[])+['土砂災害警戒情報']))
 regions={}
 ...
 Path('data').mkdir(exist_ok=True);Path('data/jma.json').write_text(json.dumps({'updated':now,'prefectures':prefs,'regions':regions,'municipalities':fmun,'sources':['JMA current warning JSON','JMA disaster XML feeds'],'note':'Fukushima municipality status is the basis for seven-region aggregation. Active landslide warning bulletins are propagated to named municipalities before aggregation.'},ensure_ascii=False,indent=2),encoding='utf-8')
